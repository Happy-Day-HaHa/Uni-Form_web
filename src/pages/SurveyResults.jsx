...
export default function SurveyResults() {
  ...
  const summary = `${analyses[0]?.counts?.length ? `첫 번째 문항에서는 ‘${[...analyses[0].counts].sort((a, b) => b.count - a.count)[0].option}’ 응답이 가장 많아요.` : '주관식 답변에서 반복되는 의견을 문항별로 확인해보세요.'} 표본이 더 모일수록 결과의 의미가 선명해집니다.`
  ...
  return <><Header /><main className="app-main result-page">
    ...
    <ResultOverview survey={survey} sampleCount={responses.length} summary={summary} />
