...
import { deleteSurvey, demoSurveys, duplicateSurvey, getMySurveys, updateSurvey } from '../services/surveyService'
  ...
  return <ServiceShell activePath="/my-surveys"><div ref={rootRef}>
    ...
    {loading ? <div className="catalog-skeleton" aria-label="설문을 불러오는 중">{Array.from({ length: 4 }, (_, index) => <div key={index}><span /><p /><i /></div>)}</div> : <section className="managed-list">{display.map((survey, index) => {
      ...
      return <article className="managed-row ui-card" key={survey.id} data-motion-reveal style={{ '--delay': `${Math.min(index, 4) * 50}ms` }}>
        <div className="managed-row__title"><span className={`service-tone--${['violet', 'amber', 'rose', 'mint', 'blue'][index % 5]}`}>{['◇', '○', '▤', '◎', '✦'][index % 5]}</span><div><div className="managed-title-line"><h2>{survey.title}</h2><em className={`survey-state survey-state--${stateKey}`}>{stateLabel}</em></div><p>{survey.description}</p><small>{survey.category || '일반'} · 약 {survey.estimated_minutes || 5}분 · 마감 {survey.deadline || '미설정'}</small></div></div>
