...
export default function SurveyCreate() {
  ...
  return <><Header /><main className="app-main create-page"><div className="page-heading"><p className="eyebrow">SURVEY WORKSPACE</p><h1>질문을 만들고,<br />함께 다듬으세요.</h1><p>왼쪽에서 직접 편집하고 오른쪽 Uni-Chat의 제안을 바로 반영할 수 있어요.</p></div>
    <section className="create-workspace">
      ...
      <aside className="ai-draft-panel"><div className="workspace-label"><span>02</span><b>UNI-CHAT</b></div><div><span>✦ AI ASSISTANT</span><h2>어떤 내용을 알아보고 싶나요?</h2><p>목적을 입력하면 질문, 보기, 순서와 표현을 함께 다듬어드려요.</p></div><textarea value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} rows="5" placeholder="예: 통학 시간과 수업 만족도의 관계를 알아보고 싶어요." /><button type="button" onClick={createAiDraft}>설문에 반영하기 →</button>{aiMessage && <small>{aiMessage}</small>}</aside>
    </section>
