export default function QuestionItem({ question, value, onChange, index }) {
  ...
  return <fieldset className="question">
    ...
    <div className="question__heading" aria-hidden="true">
      <span>{number}</span>
      <strong>{question.title}{!required && <small className="question__optional-tag">선택</small>}</strong>
    </div>
    ...
    {question.type === 'text' && <input type="text" maxLength={100} value={value || ''} onChange={(event) => onChange(event.target.value)} placeholder="자유롭게 입력해주세요. (최대 100자)" />}
