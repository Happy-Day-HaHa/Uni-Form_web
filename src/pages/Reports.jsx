...
export default function Reports() {
  ...
  const rootRef = useReveal([loading, owned.length])
  ...
  useEffect(() => {
    ...
    getMySurveys(user.id)
      ...
      .catch(() => {
        if (!active) return
        setOwned([])
        setLoadError('결과 목록을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.')
      })
      .finally(() => active && setLoading(false))
    return () => { active = false }
  }, [demoMode, reloadKey, user.id])
  ...
  return <ServiceShell activePath="/reports"><div ref={rootRef}>
    ...
    <section className="service-metrics">
      <MetricCard animate icon="◎" label="전체 응답" value={totalResponses} unit="건" note="분석 가능한 누적 응답이에요." />
      <MetricCard animate tone="mint" icon="✓" label="완료된 분석" value={reports.length} unit="개" note="결과가 준비된 설문이에요." />
      <MetricCard animate tone="violet" icon="▥" label="진행 중 설문" value={reports.filter((survey) => survey.status === 'active').length} unit="개" note="현재 응답을 모으고 있어요." />
      <MetricCard animate tone="amber" icon="↗" label="평균 완료율" value={averageCompletion} unit="%" note="목표 응답 대비 평균 수치예요." />
    </section>
    ...
    {loading ? <section className="managed-list" aria-label="결과 목록을 불러오는 중">{[0, 1, 2].map((item) => <div className="managed-row skeleton-block" key={item} />)}</section>
      : loadError ? <section className="result-empty"><span>!</span><h2>결과를 불러오지 못했어요.</h2><p>{loadError}</p><button className="ui-button" type="button" onClick={() => setReloadKey((key) => key + 1)}>다시 불러오기</button></section>
        ...
        : <section className="managed-list">{reports.slice(0, 8).map((survey, index) => {
          ...
          return <article className="managed-row ui-card" data-clickable="true" data-motion-reveal style={{ '--delay': `${Math.min(index, 3) * 35}ms` }} key={survey.id}>
