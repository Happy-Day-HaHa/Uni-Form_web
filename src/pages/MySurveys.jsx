...
export default function MySurveys() {
  ...
  useEffect(() => { getMySurveys(user.id).then(setSurveys).finally(() => setLoading(false)) }, [demoMode, user.id])
  ...
  async function confirmClose() { const survey = closeConfirmSurvey; await closeSurvey(survey.id); setSurveys((current) => current.map((item) => item.id === survey.id ? { ...item, status: 'closed' } : item)); setCloseConfirmSurvey(null); notify('설문 모집을 종료했습니다.'); setMenuId('') }
  ...
  return <ServiceShell activePath="/my-surveys"><div ref={rootRef}>
    ...
    <Modal open={Boolean(closeConfirmSurvey)} title="설문을 직접 마감할까요?" onClose={() => setCloseConfirmSurvey(null)}><p>마감한 설문은 다시 열 수 없습니다. 같은 주제로 다시 모집하려면 설문을 복제해 새로 게시해야 하며, 마감 30일 후 설문 원문은 파기됩니다.</p><div className="modal-actions"><button className="ui-button ui-button--secondary" onClick={() => setCloseConfirmSurvey(null)}>취소</button><button className="ui-button ui-button--danger" onClick={confirmClose}>마감하기</button></div></Modal>
