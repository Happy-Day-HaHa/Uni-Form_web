...
import { getSurvey, isDemoSurveyFixture, updateSurvey } from '../services/surveyService'
...
export default function SurveyManage() {
  ...
  async function closeSurvey() {
    const updated = await updateSurvey(survey.id, { status: 'closed' })
  ...
  return <ServiceShell activePath="/my-surveys"><div className="survey-manage">
    ...
    <header className="survey-manage__header"><div><span className={`survey-state survey-state--${isDraft ? 'draft' : isClosed ? 'analysis' : 'active'}`}>{statusLabel(survey.status)}</span><h1>{survey.title}</h1><p>{survey.description}</p></div></header>

    <section className="survey-manage__overview">
      <article><small>현재 상태</small><strong>{statusLabel(survey.status)}</strong></article>
      ...
      <article><small>마감일</small><strong>{survey.deadline || '미설정'}</strong></article>
    ...
    <section className="survey-manage__actions"><header><h2>설문 관리</h2><p>이 설문에 필요한 작업만 모아두었습니다.</p></header><div>
      ...
      {!isDraft && !isClosed && <button className="survey-manage-action" type="button" onClick={closeSurvey}><span>직접 마감</span><small>새 응답 모집을 종료합니다.</small><b>→</b></button>}
    </div></section>
